...
template <class TEntityData> class EntityManager : public EntityManagerBase
{
  public:
    ...
    std::vector<std::shared_ptr<Entity>>& GetEntitiesByTag(Tag tag) override { return _entityMap[tag]; }
    ...
    void Update() override
    {
        ...
        std::erase_if(_entityPairs, [](auto& entityPair) {
            ...
            assert((!isDestroyed || entity.use_count() <= 4) &&
        ...
        for (auto& entityPair : _toAdd)
        {
            ...
            _entityMap[std::get<std::shared_ptr<Entity>>(entityPair)->GetTag()].push_back(
                std::get<std::shared_ptr<Entity>>(entityPair));
