...
template <> struct std::hash<Snowglobe::Core::ECS::Tag>
{
    std::size_t operator()(const Snowglobe::Core::ECS::Tag& tag) const noexcept
    {
        return std::hash<uint32_t>()(tag.GetId());
    }
};
...
#define REGISTER_TAG(name)                                                                                             \
    ...
    {                                                                                                                  \
    inline Core::ECS::Tag name()                                                                                       \
    {                                                                                                                  \
        static Core::ECS::Tag name = Core::ECS::TagManager::GetInstance().CreateTag(#name);                            \
        return name;                                                                                                   \
    }                                                                                                                  \
    }
