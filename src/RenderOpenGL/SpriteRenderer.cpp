...
void SpriteRenderer::Init(std::shared_ptr<Core::ECS::EntityManagerBase> entityManager)
{
    ...
    auto vertexShader = shaderCompiler->GetOrCompileShader(Core::SnowFileHandle("textureUnlit.vert"));
    // auto fragmentShader = shaderCompiler->GetOrCompileShader(Core::SnowFileHandle("Sprite.frag"));
    auto fragmentShader = shaderCompiler->GetOrCompileShader(Core::SnowFileHandle("textureUnlit.frag"));
