...
namespace Snowglobe::SnowEngine
{
    ...
    class PhysicsEngine2DSystem : public SnowCore::ECS::ISystem
    {
    public:
        PhysicsEngine2DSystem(SnowCore::ECS::EntityManagerBase& entityManager) : ISystem(entityManager) {};
        
        void Update() override;

        static CollisionData OverlapPoint(const SnowCore::TransformComponent& transform, const Collider2DComponent& collider, const glm::vec2& point);
        static CollisionData Overlap(const SnowCore::TransformComponent& transformA, const Collider2DComponent& colliderA, const SnowCore::TransformComponent& transformB, const Collider2DComponent& colliderB);
    ...
    private:
        ...
        inline static glm::vec2 GetMin(const SnowCore::TransformComponent& transform) { return transform.Position - transform.Scale / 2.0f; }
        inline static glm::vec2 GetMax(const SnowCore::TransformComponent& transform) { return transform.Position + transform.Scale / 2.0f; }
        inline static float GetRadius(const SnowCore::TransformComponent& transform) { return transform.Scale.x / 2.0f; } 
    };

    inline CollisionData PhysicsEngine2DSystem::Overlap(const SnowCore::TransformComponent& transformA, const Collider2DComponent& colliderA, const SnowCore::TransformComponent& transformB, const Collider2DComponent& colliderB)
    {
    ...
    inline CollisionData PhysicsEngine2DSystem::OverlapPoint(const SnowCore::TransformComponent& transform, const Collider2DComponent& collider, const glm::vec2& point)
    {
